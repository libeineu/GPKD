...
class Group_TransformerModel(FairseqModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--enc-group', type=int, metavar='N',
                            help='Number of encoder layers included in each encoder group')
        parser.add_argument('--dec-group', type=int, metavar='N',
                            help='Number of decoder layers included in each decoder group')
        # fmt: on
    ...
    def build_model(cls, args, task):
        ...
        return Group_TransformerModel(encoder, decoder)
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        temp_list = [i for i in range(self.encoder_layers)]
        layer_list = []
        if self.training and self.enc_group != -1:
            for i in range(self.encoder_layers // self.enc_group):
                temp = temp_list[i * self.enc_group:(i + 1) * self.enc_group]
                random.shuffle(temp)
                layer_list.extend(temp)
        else:
            layer_list = [i for i in range(self.encoder_layers)]

        for layer_id,layer in enumerate(layer_list):
            x = self.layers[layer](x, encoder_padding_mask)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
        ...
        temp_list = [i for i in range(self.decoder_layers)]
        layer_list = []
        if self.training and self.dec_group != -1:
            for i in range(self.decoder_layers // self.dec_group):
                temp = temp_list[i * self.enc_group:(i + 1) * self.dec_group]
                random.shuffle(temp)
                layer_list.extend(temp)
        else:
            layer_list = [i for i in range(self.encoder_layers)]
    ...
    def __init__(self, args,layer_id):
        ...
        if args.max_relative_length==-1:
            self.self_attn = MultiheadAttention(
                ...
                dropout=args.attention_dropout,init_k = layer_id // args.group +1,
            )
        else:
        ...
        self.fc1 = Linear(self.embed_dim, args.encoder_ffn_embed_dim,layer_id)
        self.fc2 = Linear(args.encoder_ffn_embed_dim, self.embed_dim,layer_id)
...
def Linear(in_features, out_features, bias=True,layer_id=1):
    m = nn.Linear(in_features, out_features, bias)
    nn.init.xavier_uniform_(m.weight,layer_id)
    if bias:
...
def base_architecture(args):
    ...
    args.enc_group = getattr(args, 'enc_group', -1)
    args.dec_group = getattr(args, 'dec_group', -1)
...
@register_model_architecture('group_transformer', 'group_transformer_t2t_wmt_en_de')
...
@register_model_architecture('group_transformer', 'group_relative_transformer_wmt_en_de')
...
@register_model_architecture('group_transformer', 'group_relative_transformer_t2t_wmt_en_de')
...
@register_model_architecture('group_transformer', 'group_relative_transformer_test')
