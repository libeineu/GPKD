...
@register_model('layerdrop_transformer')
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        for layer_num, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.encoder_layer_dropout) or layer_num == 0 or (layer_num == len(self.layers) - 1):
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def position_dropout(self, x):
        if self.training and self.encoder_position_dropout != 0:
            position_mask = (torch.rand(x.size(0)) > self.encoder_position_dropout).view(-1,1,1).cuda().half()
            x = x * position_mask
        return x
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def position_dropout(self, x):
        if self.training and self.decoder_position_dropout != 0:
            position_mask = (torch.rand(x.size(0)) > self.decoder_position_dropout).view(-1,1,1).cuda().half()
            x = x * position_mask
        return x
...
@register_model_architecture('layerdrop_transformer', 'layerdrop_transformer')
def base_architecture(args):
    ...
    args.encoder_position_dropout = 0
    args.decoder_position_dropout = 0
...
@register_model_architecture('layerdrop_transformer', 'layerdrop_transformer_iwslt_de_en')
def layerdrop_transformer_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
...
@register_model_architecture('layerdrop_transformer', 'layerdrop_transformer_wmt_en_de')
def layerdrop_transformer_wmt_en_de(args):
    base_architecture(args)


@register_model_architecture('layerdrop_transformer', 'layerdrop_transformer_t2t_wmt_en_de')
def layerdrop_transformer_t2t_wmt_en_de(args):
...
@register_model_architecture('layerdrop_transformer', 'layerdrop_relative_transformer_wmt_en_de')
def layerdrop_relative_transformer_wmt_en_de(args):
    args.max_relative_length = 8
    args.k_only = True
    base_architecture(args)


@register_model_architecture('layerdrop_transformer', 'layerdrop_relative_transformer_t2t_wmt_en_de')
def layerdrop_relative_transformer_t2t_wmt_en_de(args):
    args.encoder_normalize_before = True
    args.decoder_normalize_before = True
    args.attention_dropout = getattr(args, 'attention_dropout', 0.1)
    args.relu_dropout = getattr(args, 'relu_dropout', 0.1)
    ...
    args.decoder_layer_dropout = 0.0
    args.encoder_position_dropout = 0.0
    args.decoder_position_dropout = 0.0
    base_architecture(args)
