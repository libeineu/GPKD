...
class LabelSmoothedAdaptiveLoss(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        nll_loss = net_output[0].new(1 if reduce else bsz).zero_()

        for i in range(len(target)):
            if target[i] is not None:
                assert (target[i].min() >= 0 and target[i].max() <= logits[i].size(1))
                #print("nll_loss:", nll_loss, "nll_loss_shape:", nll_loss.size())
                nll_loss = nll_loss + F.cross_entropy(logits[i], target[i], size_average=False, ignore_index=self.padding_idx,
                                        reduce=reduce)

        targets = orig_target.view(-1, 1)
        non_pad_mask = targets.ne(self.padding_idx)
        ...
        # print("loss:",  loss, "loss_shape:", loss.size())
